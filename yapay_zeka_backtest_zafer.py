...
print("\n==========================================================")
print("  ⚔️ QUANT AI - DUVARI YIKAN TREND FİLTRELİ MOTOR V1 ⚔️  ")
print("==========================================================")

df_model = df_final.copy()

# =====================================================================
# 📊 1. ÖZELLİK MÜHENDİSLİĞİ VE MAKRO TREND EKLEME
# =====================================================================
# Makro Trend Filtresi için 200 günlük hareketli ortalamayı hesaplıyoruz hoca
df_model['SMA_200'] = df_model['Close'].rolling(window=200).mean()
...
# SMA_200 hesaplamasından dolayı oluşan ilk 200 günlük boşluğu temizliyoruz
df_model = df_model.dropna()

X = df_model.drop(columns=[
    'Close', 'High', 'Low', 'Open', 'Volume', 'Yarin_Kapanis', 'Hedef', 'Yuzde_Getiri_Yarin',
    'Bollinger_Ust', 'Keltner_Ust', 'Donchian_Ust', 'Dividends', 'Stock Splits', 'SMA_200'
])
y = df_model['Hedef']
...
# =====================================================================
# 💸 2. TREND FİLTRELİ KESKİN NİŞANCI SİMÜLASYONU
# =====================================================================
baslangic_bakiyesi = 1000.0
bakiye = baslangic_bakiyesi
komisyon_orani = 0.001 

guven_esigi = 0.55  # Olasılık filtresi dengeli düzeyde
islem_sayisi = 0
basarili_islem = 0

print(f"💰 Başlangıç Bakiyesi: {baslangic_bakiyesi}$")
print(f"🛡️ Güven Eşiği: %{guven_esigi * 100} | 🛡️ Makro Filtre: SMA_200 Boğa Filtresi Aktif")
print("-> Akıntıya karşı olmayan, güvenli işlemler seçiliyor...\n")

for idx, row in df_backtest.iterrows():
    # 🚨 BÜYÜK AMELİYAT: Model yükseliş dese bile, fiyat SMA_200'ün altındaysa pozisyonu PAS GEÇ!
    if row['Yükseliş_Olasılığı'] >= guven_esigi and row['Close'] > row['SMA_200']:
        islem_sayisi += 1
